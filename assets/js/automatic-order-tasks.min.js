  ...
  createFields(config = null) {
    ...
    ajax_api.getPostCategories().then((data) => {
      ...
      for (let category of data) {
        ...
        if (category.nested_attributes && category.nested_attributes.length > 0) {
          // Iterate through each nested attribute and include it in the options
          for (let nestedAttribute of category.nested_attributes) {
              options += `<option value="${nestedAttribute}">${nestedAttribute}</option>`;
          }
      }
...
class SendWebhookTask extends BaseOrderTask {
  ...
  createFields(config = null) {
    ...
    const deliveryUrl = field_generator.getField('delivery_url', __('Default Delivery URL Is set for Entro ', 'aotfw-domain'), field_generator.FIELD_TYPE.PLAIN );
...
class SendEntroTask extends BaseOrderTask {
  ...
  createFields(config = null) {
    ...
    const sendingToEntro = field_generator.getField('sending_to_entro', __('Sending to Entro Dashboard', 'aotfw-domain'), field_generator.FIELD_TYPE.PLAIN);
  
   const paragraphText = "This task sends data to the Entro dashboard for further processing.";

   const $paragraph = $('<p>').text(paragraphText).css({
    'color': 'green', // Set color to green
    'font-size': '18px' // Set font size to 18px
  });
   $fieldGroup.append($paragraph);

    //$fieldGroup.append(deliveryUrl.container);
    //  $fieldGroup.append(sendingToEntro.container); 
...
class FilterOrderTask extends BaseOrderTask {
  ...
  constructor() {
    super('filterorder', 'fa-solid fa-truck', __('Filter Orders by Delivery Location', 'aotfw-domain'));
  }
  ...
  createFields(config = null) {
    ...
    const delivery_method = field_generator.getField('delivery_method', __('Choose Delivery Locations', 'aotfw-domain'), field_generator.FIELD_TYPE.SELECT_MULTIPLE);
    ...
    $fieldGroup.append(delivery_method.container);
    ...
    ajax_api.getPostCategories().then((data) => {
      ...
      const $select = delivery_method.container.find('select');
      ...
      if (config) {
        ...
        delivery_method.setValue(config.fields.delivery_method);
         const selectedValues = $select.val();
                const deliveryMethodSelected = selectedValues && selectedValues.length > 0;
                $saveButton.prop('disabled', !deliveryMethodSelected);
      }
      $('#save-changes-btn').prop('disabled', true);
    });

    delivery_method.container.find('select').on('change', function() {
      const selectedValues = $(this).val();
      const deliveryMethodSelected = selectedValues && selectedValues.length > 0;
      $('#save-changes-btn').prop('disabled', !deliveryMethodSelected);
  });
